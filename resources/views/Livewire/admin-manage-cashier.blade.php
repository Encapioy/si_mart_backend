    ...
    <div x-data="{ open: @entangle('showHistoryModal') }" x-on:open-history-modal.window="open = true"
        ...
        <div x-show="open" x-transition.scale
            ...
            <div class="overflow-y-auto flex-1 p-0 relative">
                ...
                {{-- HAPUS: Bagian <div wire:loading.flex ...> kita buang total --}}

                    {{-- KITA GANTI DENGAN INI: --}}
                    {{-- Saat loading, konten tabel cuma jadi agak transparan (opacity-50) --}}
                    {{-- pointer-events-none mencegah user klik aneh-aneh saat loading --}}
                    <div wire:loading.class="opacity-50 pointer-events-none" class="transition-opacity duration-200">
                        ...
                        @if($selectedCashierId && count($historyTopups) > 0)
                            <table class="w-full text-left text-sm">
                                <thead
                                    class="bg-slate-50 text-slate-500 font-bold border-b border-slate-100 sticky top-0 z-10 shadow-sm">
                                    <tr>
                                        <th class="p-4 pl-6 bg-slate-50">Tanggal</th>
                                        <th class="p-4 bg-slate-50">User</th>
                                        <th class="p-4 text-right pr-6 bg-slate-50">Nominal</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-100">
                                    @foreach($historyTopups as $history)
                                        {{-- wire:key wajib agar rendering Livewire stabil --}}
                        ...
                        @else
                            <div class="p-10 text-center text-slate-400 flex flex-col items-center mt-10">
