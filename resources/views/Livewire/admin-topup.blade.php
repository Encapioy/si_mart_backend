    ...
    <div class="bg-white w-full max-w-md rounded-3xl shadow-xl shadow-slate-200/50 border border-white overflow-visible relative z-10">
        ...
        <div class="px-8 pb-8">
            <form wire:submit.prevent="triggerConfirm" autocomplete="off" class="space-y-5">
                ...
                {{-- 1. PENCARIAN SISWA (Dibuat Ringan) --}}
                <div class="relative z-50">
                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Cari Akun User</label>
                    <div class="relative group">
                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                            {{-- Icon berubah jadi Spinner saat searching --}}
                        ...
                        <input type="text"
                            ...
                            placeholder="Ketik nama / username / email / no handphone"
                            autofocus
                        >
                            ...
                            <div class="absolute top-full left-0 w-full mt-2 bg-white border border-slate-100 rounded-xl shadow-xl max-h-60 overflow-y-auto custom-scroll z-50 ring-1 ring-black/5">
                                @foreach($usersFound as $u)
                                    <div wire:click="selectUser({{ $u->id }})"
                                        ...
                                        <div>
                                            <p class="font-bold text-slate-700 text-sm group-hover:text-emerald-700">{{ $u->nama_lengkap }}</p>
                                            <p class="text-xs text-slate-400">{{ $u->username }}</p>
                                        </div>
                                        <i class="fa-solid fa-check text-emerald-500 opacity-0 group-hover:opacity-100 transition-opacity"></i>
                                    </div>
                                @endforeach
                            </div>
                        @endif
                ...
                {{-- 3. VERIFIKASI KASIR (Compact Mode) --}}
                <div class="bg-slate-50 p-4 rounded-xl border border-slate-100 lg:flex gap-3 ">
                    ...
                    <div class="lg:w-1/2 w-full">
                        ...
                        <input type="password" wire:model="cashier_pin" maxlength="6"
                            class="w-full bg-white border border-slate-200 rounded-lg py-2 text-center text-xs font-bold tracking-widest focus:border-emerald-500 outline-none" placeholder="******">
                    </div>
                </div>
                @error('cashier_pin') <span class="text-red-500 text-xs text-center block">{{ $message }}</span> @enderror
                @error('cashier_id') <span class="text-red-500 text-xs text-center block">{{ $message }}</span> @enderror

                {{-- 4. TOMBOL SUBMIT (Fixed Loading State) --}}
                <button type="submit"
                    wire:loading.attr="disabled"
                    wire:target="triggerConfirm"
                    ...
                    {{-- Tampil HANYA saat triggerConfirm (bukan saat search) --}}
                    <div wire:loading wire:target="triggerConfirm" class="flex items-center justify-center gap-2" style="display: none;">
    ...
    <script>
        document.addEventListener('livewire:initialized', () => {
            ...
            Livewire.on('show-confirmation-modal', (event) => {
                // Focus ke tombol confirm otomatis biar admin bisa langsung Enter
                swalConfig.fire({
                    title: 'Konfirmasi?',
                    ...
                    reverseButtons: true,
                    focusConfirm: true // Fitur penting buat kecepatan!
                }).then((result) => {
